...
public:
	...
	void RenderOutline(Graphics& gfx) const noexcept(!_DEBUG);
	void RenderWithOutline(Graphics& gfx) const noexcept(!_DEBUG);
...
private:
	...
	mutable bool renderingOutline = false;  // Flag to indicate if we're rendering the outline
