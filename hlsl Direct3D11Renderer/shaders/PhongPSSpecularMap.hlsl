...
float4 main(float3 posViewSpace : Position, float3 normal : Normal, float2 texCoord : TexCoord) : SV_TARGET
{
    ...
    const float3 diffuse = diffuseColor * diffuseIntensity * max(0.0f, dot(lightDirection, normal));
    ...
    const float3 specular = specularColorIntensity * pow(max(0.0f, dot(reflectionVector, viewDirection)), specularPowerFromMap) * specularIntensity;
